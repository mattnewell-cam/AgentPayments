...
function nextResponse(request, env, assetsBinding = 'ASSETS') {
  if (!env[assetsBinding] || typeof env[assetsBinding].fetch !== 'function') {
    return new Response(`${assetsBinding} binding is missing.`, { status: 500 });
  }
  return env[assetsBinding].fetch(request);
}
  ...
  return {
    async fetch(request, env) {
      ...
      if (isPublicPath(url.pathname, publicPathAllowlist)) {
        return nextResponse(request, env, assetsBinding);
      }
